...
var Palavra = React.createClass({
    ...
    render: function() {
        ...
        var importante = (word == 'consectetur') ? ' importante' : '';
        var spaces = /\s/.test(word) ? ' spaces' : '';
...
var Linha = React.createClass({
    render: function() {
        ...
        var words = line.match(/(\S+)|(\s+)/g) || [''];
        var position = 0;
        
        var palavras = words.map(function(word,i) {
           var node = <Palavra key={position} word={word} ></Palavra>
...
var TextSpace = React.createClass({
    ...
    render: function() {
        ...
        var linhas = this.props.lines.map(function(line,i) {
            var words = line.match(/(\S+)|(\s+)/g)
            return <Linha key={i} line={line}></Linha>
        });
...
function render() {
    ...
    ReactDOM.render(
        <TextSpace lines={lines}></TextSpace>,
